...
setup_iommu() {
  ...
  if [[ "$NEED_REBOOT" == true ]]; then
    ...
    echo
    echo -e "${YELLOW}${BOLD}ВНИМАНИЕ: Для активации IOMMU требуется перезагрузка!${NC}"
    read -rp "Перезагрузить систему сейчас? (y/n): " REB
    if [[ "$REB" =~ ^[Yy]$ ]]; then
      ...
      echo
      msg_error "Работа скрипта приостановлена."
      echo -e "${YELLOW}Изменения внесены, но не активны. Перезагрузите сервер вручную,${NC}"
      echo -e "${YELLOW}после чего снова запустите скрипт для продолжения установки.${NC}"
      exit 0
    fi
  else
...
while true; do
  header
  echo " 1) Настроить IOMMU + Установить OpenWrt"
  echo " 2) Настроить IOMMU"
  echo " 3) Установить OpenWrt"
  echo " 0) Выход"
  echo
  read -rp " Выберите действие: " OPT

  case "$OPT" in
    1) setup_iommu; install_openwrt ;;
    2) setup_iommu ;;   
    3) install_openwrt ;;
    0) exit 0 ;;
    *) echo " Неверный выбор." ;;
